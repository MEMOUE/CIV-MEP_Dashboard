	...
	private Map<String, double[]> chargerCapital(GAMSDatabase db, String[] activites) {
		...
		try {
			...
			for (int i = 0; i < annees.length; i++) {
				...
				// Pour chaque activité énergétique (a-tnd, a-gaselec, a-hydro, a-otherelec)
				for (String activite : activites) {
					...
					try {
						...
						// Formule: (a-tnd de pk_pt * a-tnd de xf) + (a-gaselec de pk_pt * a-gaselec de xf) + (a-hydro de pk_pt * a-hydro de xf) + (a-otherelec de pk_pt * a-otherelec de xf)
				...
				System.out.println("Valeur totale du capital énergétique pour période " + periode + ": " + valeurs[i]);
			}

			// Stocker les valeurs
			resultat.put("Level", valeurs);

			System.out.println("Chargement du capital énergétique terminé. Valeurs calculées: " + Arrays.toString(valeurs));

		} catch (Exception e) {
			System.out.println("Erreur lors du chargement du capital énergétique: " + e.getMessage());
			e.printStackTrace();
